...
{
	//inicio crição d variaveis /string
	char arquivo[40];
	char cpf[40];
	char nome[40];
	char sobrenome[40];
...
int consulta()
{
	//inicio crição d vaaveis string
	...
	if(file == NULL)
	{
	printf("Não foi possivel abrir o arquivo, não localzado!.\n" );
	}
	
	while(fgets(conteudo, 200, file)!= NULL) //laço
	{
		...
		for(laco=1;laco=1;)
		{
			...
			system("cls;"); //responsavel por limpar a tela
		 	...
		 	switch(opcao) //inicio da seleção do menu
		 	{
				...
				return 0;//quebra qualquer tipo de laço ou função e encerra o sistema
				break;
